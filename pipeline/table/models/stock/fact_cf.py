...
class FactStockCF(Base, TimestampMixin):
    ...
    beginning_cash_flow = Column(Numeric(21, 0), nullable=True)  # 기초 현금 및 현금성 자산
    end_cash_flow = Column(Numeric(21, 0), nullable=True)  # 기말 현금 및 현금성 자산
    operating_cash_flow = Column(Numeric(21, 0), nullable=True)  # 영업활동현금흐름, 기업 운영으로 인한 현금흐름
    investing_cash_flow = Column(Numeric(21, 0), nullable=True)  # 투자활동현금흐름 추가투자로 인한 현금흐름
    financing_cash_flow = Column(Numeric(21, 0), nullable=True)  # 재무활동 현금흐름
    exchange_rate_cash_flow = Column(Numeric(21, 0), nullable=True)  # 환율변동효과
